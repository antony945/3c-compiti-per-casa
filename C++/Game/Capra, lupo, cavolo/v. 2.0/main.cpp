...
/*v. 2.0: si cerca di dichiarara le variabile non come globali e di passarle all'interno delle funzioni per riferimento (&). */
...
void messaggioFinale(bool capra_sx, bool lupo_sx, bool cavolo_sx);
...
void inizializzaGioco(bool& lupo_sx, bool& capra_sx, bool& cavolo_sx, bool& end_game, string intestazione);
...
void disegnaScenario(string intestazione, bool lupo_sx, bool capra_sx, bool cavolo_sx, unsigned int turni);
/* Stampa a video lo scenario nel momento in cui viene richiamata
   a seconda del valore delle variabili utilizzate. */

void interazioneUtente(unsigned char& scelta);
...
void eseguiAzione(bool& errore_logico, unsigned char scelta, unsigned int turni, bool& capra_sx, bool& cavolo_sx, bool& lupo_sx);
/* CAMBIA il valore delle variabili a seconda delle scelte fatte nella funzione 'interazioneUtente'. */

void transizioneBarca(unsigned char scelta, bool lupo_sx, bool capra_sx, bool cavolo_sx, string intestazione, unsigned int turni);
/* Stampa a video l'elemento grafico della barca che cambia sponda dopo ogni turno. */

bool controllaFine(bool capra_sx, bool lupo_sx, bool cavolo_sx, unsigned int turni);
...
bool controllaVittoria(bool capra_sx, bool lupo_sx, bool cavolo_sx);
/* Controlla se, dopo aver applicato i cambiamenti imposti dall'utente alle variabili,
   l'utente ha VINTO il gioco. Può restituire 'TRUE' (vittoria), o 'FALSE' (non vittoria). */

bool controllaSconfitta(bool capra_sx, bool lupo_sx, bool cavolo_sx, unsigned int turni);
/* Controlla se, dopo aver applicato i cambiamenti imposti dall'utente alle variabili,
...
int main(){
    ...
    inizializzaGioco(lupo_sx, capra_sx, cavolo_sx, end_game, intestazione);

    for(turni = 1; !end_game;){
        disegnaScenario(intestazione, lupo_sx, capra_sx, cavolo_sx, turni);

        if(controllaFine(capra_sx, lupo_sx, cavolo_sx, turni)){
            end_game = controllaVittoria(capra_sx, lupo_sx, cavolo_sx) || controllaSconfitta(capra_sx, lupo_sx, cavolo_sx, turni);
            continue;
        }

        interazioneUtente(scelta);
        eseguiAzione(errore_logico, scelta, turni, capra_sx, cavolo_sx, lupo_sx);

        if(!errore_logico){
            transizioneBarca(scelta, lupo_sx, capra_sx, cavolo_sx, intestazione, turni);
            turni++;
        }
    }

    messaggioFinale(capra_sx, lupo_sx, cavolo_sx);
...
bool controllaFine(bool capra_sx, bool lupo_sx, bool cavolo_sx, unsigned int turni){ //IMPLEMENTATA (v. 2.0)
    bool controlla_fine = false;

    if(controllaVittoria(capra_sx, lupo_sx, cavolo_sx) || controllaSconfitta(capra_sx, lupo_sx, cavolo_sx, turni))
        controlla_fine = true;

    return controlla_fine; // restituisce alla funzione il valore di controlla_fine
}

bool controllaVittoria(bool capra_sx, bool lupo_sx, bool cavolo_sx){ //IMPLEMENTATA (v. 2.0)
...
bool controllaSconfitta(bool capra_sx, bool lupo_sx, bool cavolo_sx, unsigned int turni){ //IMPLEMENTATA (v. 2.0)
...
void inizializzaGioco(bool& lupo_sx, bool& capra_sx, bool& cavolo_sx, bool& end_game, string intestazione){ //IMPLEMENTATA (v. 2.0)
...
void disegnaScenario(string intestazione, bool lupo_sx, bool capra_sx, bool cavolo_sx, unsigned int turni){ //IMPLEMENTATA (v. 2.0)
    system("CLS");
...
void interazioneUtente(unsigned char& scelta){ //IMPLEMENTATA (v. 2.0)
...
void eseguiAzione(bool& errore_logico, unsigned char scelta, unsigned int turni, bool& capra_sx, bool& cavolo_sx, bool& lupo_sx){ //IMPLEMENTATA (v. 2.0)
    errore_logico = false;
...
void transizioneBarca(unsigned char scelta, bool lupo_sx, bool capra_sx, bool cavolo_sx, string intestazione, unsigned int turni){ //IMPLEMENTATA (v. 2.0)
    string barca("(___)");
...
void messaggioFinale(bool capra_sx, bool lupo_sx, bool cavolo_sx){ //IMPLEMENTATA (v. 2.0)
    cout << endl << "________________________________________________________________________________" << endl;
    if (controllaVittoria(capra_sx, lupo_sx, cavolo_sx)){
